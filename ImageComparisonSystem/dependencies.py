...
class DependencyChecker:
    ...
    def check_all(cls, verbose: bool = True) -> Tuple[bool, Dict[str, dict]]:
        ...
        for dep in cls.DEPENDENCIES:
            ...
            if verbose:
                ...
                if is_installed:
                    ...
                    logger.info(
                        f"[{status:8s}] {dep.package_name:20s} {version_str:15s} - {dep.description}"
                    )
                else:
                    logger.warning(
                        f"[{status:8s}] {dep.package_name:20s} MISSING - {dep.description}"
                    )
        ...
        if verbose:
            ...
            if all_satisfied:
                ...
                logger.warning(
                    "Some dependencies are missing. See installation instructions below."
                )
    ...
    def generate_install_instructions(cls, results: Dict[str, dict]) -> str:
        ...
        instructions.append(
            "   pip download "
            + " ".join([d.pip_install for d in missing])
            + " -d packages/"
        )
        instructions.append("")
        ...
        instructions.append(
            ...
            + " ".join([d.package_name for d in missing])
        )
        instructions.append("")
    ...
    def check_and_exit_if_missing(cls, skip_tkinter: bool = False):
        ...
        if skip_tkinter:
            cls.DEPENDENCIES = [
                d for d in cls.DEPENDENCIES if d.package_name != "tkinter"
            ]
    ...
    def save_requirements_freeze(cls, output_file: str = "requirements-freeze.txt"):
        ...
        try:
            ...
            logger.info(
                "Use 'pip install -r requirements-freeze.txt' to install exact versions"
            )
        except Exception as e:
