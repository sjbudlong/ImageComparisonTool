...
def check_python_version():
    ...
    if current >= required:
        print(f"✓ Python {current[0]}.{current[1]} meets requirement (>= {required[0]}.{required[1]})")
        return True
    else:
        print(f"✗ Python {current[0]}.{current[1]} does not meet requirement (>= {required[0]}.{required[1]})")
