...
class MarkdownExporter:
    ...
    def export_summary(
        self, results: List[ComparisonResult], base_path: Path = None
    ) -> Path:
        ...
        try:
            ...
            avg_diff = (
                sum(r.percent_different for r in results) / total if total > 0 else 0
            )
    ...
    def _group_results_by_subdirectory(
        self, results: List[ComparisonResult], base_path: Path = None
    ) -> List[tuple]:
